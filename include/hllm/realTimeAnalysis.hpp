...
class RealTimeAnalysis
{
  ...
  RealTimeAnalysis(const std::string ip = "0.0.0.0", const size_t port = 5003)
    : _ip(ip),
      _port(port)
  {
    ...
    _srv_thread = std::thread([this]() { _svr.listen(_ip, _port); });
  }

  ~RealTimeAnalysis()
  {
    _svr.stop();                                    // tell the server to stop listening
    if (_srv_thread.joinable()) _srv_thread.join(); // clean shutdown
  }
