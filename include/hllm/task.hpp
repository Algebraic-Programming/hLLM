...
class Task final : public taskr::Task
{
  ...
  Task(const hLLM::configuration::Task taskConfig, const taskFunction_t &function, const taskr::taskId_t taskId, taskr::Function *entryPoint)
  ...
  __INLINE__ const std::shared_ptr<HiCR::LocalMemorySlot> getInput(const std::string &inputName)
  {
    ...
    if (_inputs[inputName] == nullptr)
      HICR_THROW_RUNTIME(
        "Task '%s' trying to access input '%s' which has not been provided. This must be a bug in hLLM.\n", _taskConfig.getFunctionName().c_str(), inputName.c_str());
  ...
  __INLINE__ void setOutput(const std::string &outputName, const std::shared_ptr<HiCR::LocalMemorySlot> &memorySlot)
  {
    ...
    if (_outputsSent.contains(outputName) == true) HICR_THROW_RUNTIME("Task '%s' is setting output '%s' twice.\n", _taskConfig.getFunctionName().c_str(), outputName.c_str());
  ...
  __INLINE__ bool isReady() const
  {
    for (const auto &input : _inputs)
      if (input.second == nullptr) return false;
    return true;
  }
  ...
  __INLINE__ void setInput(const std::string &inputName, const std::shared_ptr<HiCR::LocalMemorySlot> token)
  {
    ...
    if (_inputs.contains(inputName) == false)
      HICR_THROW_RUNTIME(
        "Task '%s' trying to set input '%s' which has not been declared for this task. This must be a bug in hLLM.\n", _taskConfig.getFunctionName().c_str(), inputName.c_str());
