...
class Replica final : public Base
{
      ...
      _registeredFunctions(registeredFunctions)
  {
    ...
    _coordinatorControlInput =
      std::make_shared<edge::Input>(*_controlEdgeConfig, edge::edgeType_t::coordinatorToReplica, edge::Base::controlEdgeIndex, _partitionIdx, _partitionIdx, _replicaIdx);
    _coordinatorControlOutput =
    ...
    for (const auto &task : tasks)
      for (const auto &taskInput : task->getInputs())
        for (size_t edgePos = 0; edgePos < _inputEdges.size(); edgePos++)
          ...
          {
            _taskInputEdgePositions[task->getFunctionName()].push_back(edgePos);
            break;
          }
    ...
    for (const auto &input : _inputEdges)
      for (const auto &task : tasks)
        for (const auto &taskInput : task->getInputs())
        {
          ...
          {
            _inputEdgeTaskDependencies[inputName].push_back(task->getFunctionName());
            break;
          }
        }
    ...
    for (const auto &task : tasks)
      for (const auto &taskOutput : task->getOutputs())
      {
        ...
        // Sanity check
        if (foundEdge == false)
          HICR_THROW_RUNTIME("[Replica %lu / %lu] Could not find the edge for output: %s. This must be a bug in hLLM", _partitionIdx, _replicaIdx, taskOutput.c_str());
      }
  }
  ...
  __INLINE__ void runTaskRFunction(taskr::Task *taskrTask)
  {
    ...
    for (const auto &output : taskConfig.getOutputs())
    {
      ...
      if (_taskOutputEdgePositions.contains(output) == false)
        HICR_THROW_RUNTIME("[Replica %lu / %lu] Could not find output: %s's position...", _partitionIdx, _replicaIdx, output.c_str());
    ...
    if (_activeJob != nullptr)
    {
      ...
      for (const auto &output : _activeJob->getOutputEdges())
        if (output.isSatisfied() == false) isJobFinished = false;
  ...
  __INLINE__ void initializeImpl() override
  {
    ...
    subscribeEdgeMessageHandler(hLLM::Role::edgeHandlerSubscription_t{
      hLLM::messages::messageTypes::heartbeat, _coordinatorControlInput, [this](const std::shared_ptr<edge::Input> edge, const hLLM::edge::Message &message) {
  ...
  __INLINE__ void inputDataMessageHandler(const std::shared_ptr<edge::Input> edge, const std::shared_ptr<hLLM::messages::Data> message)
  {
    ...
    if (_activeJob != nullptr)
      if (promptId != _activeJob->getPromptId())
        HICR_THROW_RUNTIME("[Replica %lu/%lu] Received data for prompt %lu/%lu, edge '%s' but currently prompt %lu/%lu is running.\n",
                           _partitionIdx,
                           _replicaIdx,
                           promptId.first,
                           promptId.second,
                           edge->getEdgeConfig().getName().c_str(),
                           _activeJob->getPromptId().first,
                           _activeJob->getPromptId().second);
  ...
  __INLINE__ void startJob(std::shared_ptr<Job> &job)
  {
    ...
    for (const auto &task : tasks)
    {
      ...
      if (_registeredFunctions.contains(taskFunctionName) == false)
        HICR_THROW_LOGIC("The requested function name '%s' is not registered. Please register it before running the hLLM.", taskFunctionName.c_str());
      ...
      auto taskInputsCheck = [&]() {
        ...
        for (const auto edgePos : taskInputEdgePositions)
          if (_activeJob->getInputEdges()[edgePos].isSatisfied() == false) return false;
