...
class Base : public hLLM::Role
{
  ...
  class Job final
  {
    ...
    class Edge final
    {
      ...
      Edge(const edgeInfo_t edgeInfo)
        : _edgeInfo(edgeInfo)
      {}
      ~Edge() = default;
      ...
      __INLINE__ void setSatisfied(const bool value = true)
      {
        if (value == _isSatisfied)
          HICR_THROW_RUNTIME("The specified edge has already been set as '%s' for edge '%s'. This is a bug in hLLM.", value ? "True" : "False", _edgeInfo.config->getName().c_str());
        _isSatisfied = value;
      }
    ...
    Job(const Prompt::promptId_t promptId, const std::vector<edgeInfo_t> &inputEdges, const std::vector<edgeInfo_t> &outputEdges)
      : _promptId(promptId)
    {
  ...
  Base(const configuration::Deployment deployment, const configuration::Partition::partitionIndex_t partitionIdx, taskr::Runtime *const taskr)
      ...
      _partitionIdx(partitionIdx)
  {
    ...
    for (configuration::Edge::edgeIndex_t edgeIdx = 0; edgeIdx < edgeConfigs.size(); edgeIdx++)
    {
      ...
      if (edgeConfig->getConsumer() == partitionName && edgeConfig->isResultEdge() == false)
      {
        ...
        // Sanity check
        if (producerFound == false)
          HICR_THROW_RUNTIME("Could not find index of producer '%s' for edge '%s'. This is a bug in hLLM.", producerPartitionName.c_str(), edgeConfig->getName().c_str());
      ...
      if (edgeConfig->getProducer() == partitionName && edgeConfig->isPromptEdge() == false)
      {
        ...
        if (consumerFound == false)
          HICR_THROW_RUNTIME("Could not find index of consumer '%s' for edge '%s'. This is a bug in hLLM.", consumerPartitionName.c_str(), edgeConfig->getName().c_str());
