...
class Coordinator final : public Base
{
  ...
  class Replica final
  {
    ...
    Replica() = delete;
    Replica(const configuration::Partition::partitionIndex_t partitionIndex,
        ...
        _replicaIndex(replicaIndex)
    {
      ...
      _controlInput =
        std::make_shared<edge::Input>(*controlEdgeConfig, edge::edgeType_t::replicaToCoordinator, edge::Base::controlEdgeIndex, _partitionIndex, _partitionIndex, _replicaIndex);
      _controlOutput =
        std::make_shared<edge::Output>(*controlEdgeConfig, edge::edgeType_t::coordinatorToReplica, edge::Base::controlEdgeIndex, _partitionIndex, _partitionIndex, _replicaIndex);
    }
  ...
  Coordinator(const configuration::Deployment deployment, const configuration::Partition::partitionIndex_t partitionIdx, taskr::Runtime *const taskr)
    : Base(deployment, partitionIdx, taskr)
  {
    ...
    for (const auto &edge : _outputEdges)
    {
      ...
      _partitionDataOutputs.push_back(
        std::make_shared<edge::Output>(*edgeConfig, edgeType, edgeIdx, producerPartitionIdx, consumerPartitionIdx, edge::Base::coordinatorReplicaIndex));
  ...
  __INLINE__ void initializeImpl() override
  {
    ...
    for (const auto &replica : _replicaMap)
      subscribeEdgeMessageHandler(hLLM::Role::edgeHandlerSubscription_t{
        hLLM::messages::messageTypes::heartbeat, replica.second->getControlInput(), [this](const std::shared_ptr<edge::Input> edge, const hLLM::edge::Message &message) {
          heartbeatMessageHandler(edge, std::make_shared<hLLM::messages::Heartbeat>(message));
        }});

    // Subscribing control edges to the message service for my replicas's ready message
    for (const auto &replica : _replicaMap)
      subscribeEdgeMessageHandler(hLLM::Role::edgeHandlerSubscription_t{
        hLLM::messages::messageTypes::replicaReady, replica.second->getControlInput(), [this](const std::shared_ptr<edge::Input> edge, const hLLM::edge::Message &message) {
          replicaReadyMessageHandler(edge, std::make_shared<hLLM::messages::ReplicaReady>(message));
        }});

    // Subscribing data input edges to the message service for my replicas results (outputs)
    for (const auto &replica : _replicaMap)
      for (const auto &dataEdge : replica.second->getDataInputs())
        subscribeEdgeMessageHandler(
          hLLM::Role::edgeHandlerSubscription_t{hLLM::messages::messageTypes::data, dataEdge, [this](const std::shared_ptr<edge::Input> edge, const hLLM::edge::Message &message) {
                                                  outputDataMessageHandler(edge, std::make_shared<hLLM::messages::Data>(message));
                                                }});
    ...
    for (const auto &dataEdge : _partitionDataInputs)
      subscribeEdgeMessageHandler(
        hLLM::Role::edgeHandlerSubscription_t{hLLM::messages::messageTypes::data, dataEdge, [this](const std::shared_ptr<edge::Input> edge, const hLLM::edge::Message &message) {
                                                inputDataMessageHandler(edge, std::make_shared<hLLM::messages::Data>(message));
                                              }});
  ...
  __INLINE__ void outputDataMessageHandler(const std::shared_ptr<edge::Input> edge, const std::shared_ptr<hLLM::messages::Data> message)
  {
    ...
    for (const auto &output : job->getOutputEdges())
      if (output.isSatisfied() == false)
      {
        isJobFinished = false;
        break;
      };
  ...
  __INLINE__ void jobManagementService()
  {
    ...
    for (const auto &input : job->getInputEdges())
      if (input.isSatisfied() == false)
      {
        isJobReady = false;
        break;
      };
