...
class RequestManager final : public hLLM::Role
{
  ...
  RequestManager(const configuration::Deployment deployment, taskr::Runtime *const taskr)
      ...
      prev_num_responses(0)
  {
    ...
    for (size_t edgeIdx = 0; edgeIdx < edgeConfigs.size(); edgeIdx++)
    {
      ...
      if (edgeConfig->isPromptEdge() == true)
      {
        ...
        _promptOutputEdge = std::make_shared<edge::Output>(
          *edgeConfig, edge::edgeType_t::requestManagerToCoordinator, edgeIdx, _promptConsumerPartitionIdx, _promptConsumerPartitionIdx, edge::Base::coordinatorReplicaIndex);
    ...
    for (size_t edgeIdx = 0; edgeIdx < edgeConfigs.size(); edgeIdx++)
    {
      ...
      if (edgeConfig->isResultEdge() == true)
      {
        ...
        _resultInputEdge = std::make_shared<edge::Input>(
          *edgeConfig, edge::edgeType_t::coordinatorToRequestManager, edgeIdx, _resultProducerPartitionIdx, _resultProducerPartitionIdx, edge::Base::coordinatorReplicaIndex);
  ...
  __INLINE__ void initializeImpl() override
  {
    ...
    subscribeEdgeMessageHandler(hLLM::Role::edgeHandlerSubscription_t{
      hLLM::messages::messageTypes::data, _resultInputEdge, [this](const std::shared_ptr<edge::Input> edge, const hLLM::edge::Message &message) {
        responseDataMessageHandler(edge, std::make_shared<hLLM::messages::Data>(message));
      }});
  ...
  __INLINE__ void responseDataMessageHandler(const std::shared_ptr<edge::Input> edge, const std::shared_ptr<hLLM::messages::Data> message)
  {
    ...
    if (_activePromptMap.contains(promptId) == false)
      HICR_THROW_RUNTIME("Prompt map entry for prompt %lu/%lu not found. This must be a bug in hLLM", promptId.first, promptId.second);
    ...
    std::string json_data = "{\n"
                            ...
                            "  \"status\": \"active\",\n"
                            "  \"num_responses\": " +
                            std::to_string(num_responses) +
                            ",\n"
                            "  \"avg_responses_per_min\": " +
                            std::to_string(avg_res_per_minute) +
                            "\n"
                            "  }\n"
                            "}";
