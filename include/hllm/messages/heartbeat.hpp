...
class Heartbeat final : public Base
{
  ...
  Heartbeat() = default;
  Heartbeat(const edge::Message &rawMessage)
    : Base()
  {
    decode(rawMessage);
  }
  ...
  __INLINE__ void decode(const edge::Message &rawMessage) override
  {
    ...
    if (signatureSize != _signature.size() + 1)
      HICR_THROW_RUNTIME(
        "Heartbeat message has signature incorrect size (%lu != %lu, Recevied: '%s'). This is a bug in hLLM", signatureSize, _signature.size() + 1, signature.c_str());
