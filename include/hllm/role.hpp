...
class Role
{
  ...
  Role(const configuration::Deployment deployment, taskr::Runtime *const taskr)
  ...
  __INLINE__ void edgeSubscriptionListeningService()
  {
    for (const auto &edge : _subscribedEdges)
    {
      ...
      if (edge->hasMessage())
      {
        ...
        if (_subscriptionToHandlerMap.contains(subscriptionMapKey) == false)
          HICR_THROW_RUNTIME("Edge Idx %lu cointains message of type %lu that has no subscribed handler.\n", edgeIdx, messageType);
