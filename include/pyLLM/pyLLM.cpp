...
bool initialize_engine_from_python(LLMEngine &llmengine, const std::vector<std::string> &args)
{
  ...
  for (const auto &s : args) { argv_cstr.push_back(const_cast<char *>(s.c_str())); }
...
void convert_numbers_to_unsigned(nlohmann::json &j)
{
  if (j.is_object())
  {
    for (auto &el : j.items()) { convert_numbers_to_unsigned(el.value()); }
  }
  else if (j.is_array())
  {
    for (auto &el : j) { convert_numbers_to_unsigned(el); }
  }
  else if (j.is_number())
  {
    int64_t val = j.get<int64_t>();
    if (val < 0) { throw std::runtime_error("Negative value found where unsigned expected."); }
...
PYBIND11_MODULE(llmEngine, m)
{
  ...
  py::class_<token_py>(m, "token_py").def_readonly("success", &token_py::success).def_readonly("buffer", &token_py::buffer);
}
