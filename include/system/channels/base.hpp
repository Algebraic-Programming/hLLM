...
class Base
{
  ...
  {
    if (_config.payloadCommunicationManager == nullptr) HICR_THROW_LOGIC("Required HiCR object 'PayloadCommunicationManager' not provided for channel '%s'", _config.name);
    if (_config.payloadMemoryManager == nullptr) HICR_THROW_LOGIC("Required HiCR object 'PayloadMemoryManager' not provided for channel '%s'", _config.name);
    if (_config.payloadMemorySpace == nullptr) HICR_THROW_LOGIC("Required HiCR object 'PayloadMemorySpace' not provided for channel '%s'", _config.name);
    if (_config.coordinationCommunicationManager == nullptr)
      HICR_THROW_LOGIC("Required HiCR object 'CoordinationCommunicationManager' not provided for channel '%s'", _config.name);
    if (_config.coordinationMemoryManager == nullptr) HICR_THROW_LOGIC("Required HiCR object 'CoordinationMemoryManager' not provided for channel '%s'", _config.name);
    if (_config.coordinationMemorySpace == nullptr) HICR_THROW_LOGIC("Required HiCR object 'CoordinationMemorySpace' not provided for channel '%s'", _config.name);
  }

  static __INLINE__ channelConfig_t buildChannelConfig(const hLLM::configuration::Edge &edgeConfig)
  {
