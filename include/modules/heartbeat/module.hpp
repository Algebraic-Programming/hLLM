...
class Module final : public hLLM::modules::Module
{
  ...
  using input_t       = std::shared_ptr<hLLM::system::channels::Input>;
  using output_t      = std::shared_ptr<hLLM::system::channels::Output>;
  using message_t     = hLLM::system::channels::Message;
  ...
  [[nodiscard]] __INLINE__ std::vector<hLLM::modules::channelDispatcher::Subscription> buildSubscriptions()
  {
    std::vector<hLLM::modules::channelDispatcher::Subscription> out;
  ...
  __INLINE__ void checkTimeouts()
  {
    ...
    {
      if (_lastSeen[instanceId] == std::chrono::steady_clock::time_point::min()) continue; // unknown until first heartbeat
